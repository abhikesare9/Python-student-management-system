...
class School_Portal():
    ...
    def add_record(self):
        if self.validation():
            query = 'INSERT INTO studentinfo VALUES (NULL,?,?,?,?,?,?)'
    ...
    def delete_record(self):
        ...
        query = 'DELETE FROM studentinfo WHERE ID = ?'
    ...
    def edit_record(self,new_fname,fname,new_lname,lname,new_uname,uname,new_email,email,new_subject,subject,new_age,age):
        query = 'UPDATE studentinfo SET Firstname=?, Lastname=?, Username=?, Email=?, Subject=?, Age=? WHERE Firstname=?' \
